...
const crypto = (require('../node_modules/cryptojs-atom/cryptojs')).Crypto;

module.exports = {

  activate(state) {
    this.secretKeyView = new secretKeyView(state.secretKeyViewState);
    this.secretkeyPanel = atom.workspace.addModalPanel({item: this.secretKeyView.getElement(), visible: false});

    /* Frequency Analysis */
    atom.commands.add('atom-text-editor', { 'encrypt-decrypt:frequency-analysis': () => this.frequencyanalysis()


    /* Tranform text */
  }
    );
    atom.commands.add('atom-text-editor', {'encrypt-decrypt:transform-charToCode': () => this.transformcharToCode()});
    atom.commands.add('atom-text-editor', {'encrypt-decrypt:transform-codeToChar': () => this.transformcodeToChar()});
    atom.commands.add('atom-text-editor', {'encrypt-decrypt:transform-toUpperCase': () => this.transformtoUpperCase()});
    atom.commands.add('atom-text-editor', { 'encrypt-decrypt:transform-toLowerCase': () => this.transformtoLowerCase()

    /* Caesar */
  }
    );
    atom.commands.add('atom-text-editor', {'encrypt-decrypt:encrypt-caesar': () => this.encryptcaesar()});
    atom.commands.add('atom-text-editor', { 'encrypt-decrypt:decrypt-caesar': () => this.decryptcaesar()

    /* Vigenere */
  }
    );
    atom.commands.add('atom-text-editor', {'encrypt-decrypt:encrypt-vigenere': () => this.encryptvigenere()});
    atom.commands.add('atom-text-editor', { 'encrypt-decrypt:decrypt-vigenere': () => this.decryptvigenere()
    /* AES */
  }
    );
    atom.commands.add('atom-text-editor', {'encrypt-decrypt:encrypt-aes': () => this.encryptaes()});
    atom.commands.add('atom-text-editor', { 'encrypt-decrypt:decrypt-aes': () => this.decryptaes()
    /* DES */
  }
    );
    atom.commands.add('atom-text-editor', {'encrypt-decrypt:encrypt-des': () => this.encryptdes()});
    atom.commands.add('atom-text-editor', { 'encrypt-decrypt:decrypt-des': () => this.decryptdes()
    ...
    /* HASH */
    atom.commands.add('atom-text-editor', {'encrypt-decrypt:encrypt-md5': () => this.encryptmd5()});
    atom.commands.add('atom-text-editor', {'encrypt-decrypt:encrypt-sha1': () => this.encryptsha1()});
    return atom.commands.add('atom-text-editor', {'encrypt-decrypt:encrypt-sha256': () => this.encryptsha256()});
  },
  ...
  transformcharToCode() {
    ...
    if(lengthText > 0) {
        ...
        return this.replaceText(newStr.substring(0, newStr.length-1), {'select': true});
      }
  },
  ...
  transformcodeToChar() {
    ...
    if(lengthText > 0) {
        ...
        return this.replaceText(newStr , {'select': true});
      }
  },

  transformtoUpperCase() {
    const text = this.getTextSelected();
    return this.replaceText(text.toUpperCase() , {'select': true});
  },

  transformtoLowerCase() {
    const text = this.getTextSelected();
    return this.replaceText(text.toLowerCase() , {'select': true});
  },
  ...
  encryptshift(length, toUp) {
    ...
    return this.replaceText(encryptedText, {'select': true});
  },
  ...
  submitKey() {
    this.secretkeyPanel.hide();
    this.secretkeyPanel.reset();

    const algo = this.secretKeyView.getAlgo();
    switch (algo) {
      case "vigenere": this.vigenereFromInput(); break;
      case "aes": this.aesFromInput(); break;
      case "des": this.desFromInput(); break;
    }
  ...
  encryptmd5() {
    ...
    return this.replaceText(hash, {'select': true});
  },
